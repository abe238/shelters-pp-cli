...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".local", "share", "shelters-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("SHELTERS_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("SHELTERS_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "github.com/abe238/shelters-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.local/share/shelters-pp-cli/feedback.jsonl.
When ` + "`SHELTERS_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`SHELTERS_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  shelters-pp-cli feedback list
  shelters-pp-cli feedback list --limit 5
  shelters-pp-cli feedback list --json`,
