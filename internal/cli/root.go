...
Highlights (not in the official API docs):
  ...
  • shelters   Open shelters flattened from the feed and filterable by state, pets, ADA, wheelchair, managing org, status, county/parish, and confirmed onsite generator; each shelter best-effort enriched with FEMA'…
...
Run 'shelters-pp-cli doctor' to verify auth and connectivity.`,
	...
	rootCmd.SetVersionTemplate("shelters-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().BoolVar(&flags.quiet, "quiet", false, "Suppress output; the exit code communicates the result")
	...
	rootCmd.PersistentFlags().BoolVar(&flags.noEnrich, "no-enrich", false, "Skip FEMA_NSS/0 enrichment; return only the OpenShelters spine fields (faster, one fewer request)")
	...
	rootCmd.AddCommand(newSheltersPromotedCmd(flags))
	rootCmd.AddCommand(newNovelShelterCmd(flags))
	rootCmd.AddCommand(newNovelNearCmd(flags))
	rootCmd.AddCommand(newNovelCapacityCmd(flags))
	rootCmd.AddCommand(newNovelBriefCmd(flags))
	rootCmd.AddCommand(newNovelGisLinksCmd(flags))
	rootCmd.AddCommand(newNovelCreditsCmd(flags))
