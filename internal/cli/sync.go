...
Resource scoping:
  ...
  shelters-pp-cli sync

  # Sync specific resources only
  shelters-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  shelters-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  shelters-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  shelters-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  shelters-pp-cli sync --latest-only`,
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/shelters-pp-cli/data.db)")
...
func syncResourcePath(resource string) (string, error) {
	paths := map[string]string{
		"shelters": "/arcgis/rest/services/NSS/OpenShelters/FeatureServer/0/query",
	}
