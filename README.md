...
- **Two feeds, unioned:** the FEMA National Shelter System OpenShelters feed (`gis.fema.gov`) is the spine; the CLI also fetches the American Red Cross Emergency-Action feed (`services.arcgis.com`, the `redcross.org` map's source) and **unions** the two, deduped on normalized name + state + ZIP. FEMA is synchronized downstream of Red Cross (every morning, then every 20 minutes), so a freshly opened shelter can appear in Red Cross up to a day before FEMA. Neither feed alone is complete; the union is. Every shelter carries a `source` field recording which feeds it came from (`fema`, `redcross`, or `fema+redcross`), with `+occupancy` appended (or a bare `occupancy`) once live occupancy is merged in. Both feeds cover the United States and its territories only, not other countries.
- **Live occupancy (current population):** the OpenShelters spine and the Red Cross map feed both carry a null population on their public mirrors, so the CLI also folds in the American Red Cross **Open_Shelters** operational layer (`services.arcgis.com`), the one feed that reports the current `total_population`, the general/medical/other/pet breakdown, the evacuation/post-impact capacity, and the driving incident. It is joined onto the union by name + state + ZIP (its id is an ArcGIS-internal id, not FEMA's `shelter_id`), so it fills the headcount onto FEMA and Red Cross rows alike and surfaces any shelter present only there. Capacity is often blank in blue-sky periods; population then has no denominator, which is why `capacity` honestly reports utilization as unknown.
...
- **Best-effort honesty:** the Red Cross union, the FEMA_NSS enrichment, and the live occupancy merge are all overlays on the authoritative FEMA spine. If any is skipped (`--no-enrich`, offline) or fails, the command degrades gracefully (null fields) with an explanatory `red_cross` / `enrichment` / `occupancy` note, never a fabricated, misattributed, or dropped value. No personal contact columns (organizer name/phone/email, mailing address) are ever requested.
